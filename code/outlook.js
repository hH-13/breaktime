...
function getEvents() {
    return document
        ...
        .querySelectorAll("div[aria-description]");
}
...
function gameFinishModal({
    ...
    const modal = createElt({ classList: ["decline-events-modalol"] });

    const dismiss = () => {
        console.log("DISMISSING");
        modal.style.animation = "fade-out 0.5s ease both";
        bg.classList.remove('modal_bg_blur');
        setTimeout(() => {
            modal.remove();
        }, 500);
    };
    ...
    const question = createElt({
        classList: ["decline-events-modal-questionol"],
        kind: "span",
        parent: modal,
    ...
    const choices = createElt({
        classList: ["decline-events-modal-choicesol"],
        parent: modal,
    });

    const noButton = createElt({
        classList: ["decline-events-choiceol", "decline-events-choice-restart"],
        kind: "button",
        parent: choices,
        textContent: "Restart",
        onSubmit: restartLoop,
    });
    const yesButton = createElt({
        classList: ["decline-events-choiceol", "decline-events-choice-dismiss"],
        kind: "button",
        parent: choices,
        textContent: "Dismiss",
    ...
    const bg = document.querySelector("div[id='app']");
    bg.classList.add('modal_bg_blur');
...
function main() {
    ...
    const grid = mainElt.children[0].children[0].children[0];
    ...
    //hH: confirm outlook works similarly
    ...
    const endModal = (text) => {
        ...
        gameFinishModal({
            titleText: text,
            restartGame: main,
            exitGame: signalToChromeThatWeAreDone,
        })
    };
